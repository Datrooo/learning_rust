...
pub struct CreateTodo {
    #[validate(length(
        min = 1,
        max = 500,
        message = "Task must be between 1 and 500 characters"
    ))]
    pub task: String,
...
impl Todo {
    pub async fn all<C: GenericClient>(client: &C) -> Result<Vec<Todo>, tokio_postgres::Error> {
        let rows = client
    ...
    pub async fn get_by_id<C: GenericClient>(
        client: &C,
        id: i32,
    ) -> Result<Option<Todo>, tokio_postgres::Error> {
        let row = client
            .query_opt(
                "SELECT id, task, is_finished FROM todo WHERE id = $1",
                &[&id],
            )
            .await?;
    ...
    pub async fn create<C: GenericClient>(
        client: &C,
        data: CreateTodo,
    ) -> Result<Todo, tokio_postgres::Error> {
    ...
    pub async fn delete<C: GenericClient>(
        ...
        let rows_affected = client.execute("DELETE FROM todo WHERE id = $1", &[&id]).await?;
        Ok(rows_affected > 0)
    }
}
